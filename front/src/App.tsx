        ...
        <ChatContainer>
          <>
            ...
            {renderList.map((item: string, index: number) => {
              if (index === 2) {
                return (
                  <ChatMessages key={index} src={younger}>
                    <>{item}</>
                  </ChatMessages>
                );
              } else {
                return (
                  <ChatMessages key={index} src={older}>
                    <>{item}</>
                  </ChatMessages>
                );
              }
                ...
                {currentQuestion?.option ? (
                  <ButtonGroup color="primary" variant="contained">
                    {currentQuestion?.list.map((item) => {
                      return (
                        <Button key={item} onClick={handleClick} value={item}>
                          {item}
                        </Button>
                      );
                    })}
                  ...
                  <>
                    <ChatMessages src={younger}>
                      <>
                        「{currentQuestion?.example[0]}」とか「
                        {currentQuestion?.example[1]}」とか
                      </>
                    </ChatMessages>
                    <ChatMessages src={younger}>
                      <>↓に入力してね</>
            ...
            {gotRecommend && (
              <div className="question">
                ...
                <ChatMessages src={older}>
                  <>{recommend?.drink.reason}</>
                </ChatMessages>
                <ChatMessages src={younger}>
                  <>{recommend?.food.name}</>
                </ChatMessages>
                <ChatMessages src={younger}>
                  <>{recommend?.food.reason}</>
                </ChatMessages>
